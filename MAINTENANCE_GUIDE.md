...
**维护建议**：
- `DEFAULT_PORT` 硬编码为 3000，但实际端口由 `config.local.json` 决定。`startServer()` 中日志输出的 `Close this window to stop localhost:${DEFAULT_PORT}` 可能与实际端口不符。建议从配置读取后回写。
- `openDefaultBrowser()` 的 fallback timer（5 秒）在服务启动慢时可能过早打开浏览器，建议改为只在收到 `[EasyMC:PORT]` 标记时打开。
...
- **upload 文件大小限制**：`MAX_UPLOAD_BYTES` 定义在 `socket.on('agent:upload_file')` 内部，作用域过窄。建议提取到配置或模块级常量。
- **`stopActiveChatRun`** 在 `disconnect` 中调用但 `activeChatRun` 可能为 `null`（首次连接未发消息），`stopActiveChatRun` 内部已做 `if (!activeChatRun) return` 保护，可以。
- **缺少全局错误处理**：Socket.IO 的 `connection` 回调内如果抛异常，没有全局 catch。建议用 `process.on('uncaughtException')` 兜底或给每个 handler 包 try/catch。
...
**维护建议**：
- **API Key 明文存储**：`config.local.json` 中 `ai.apiKey` 以明文 JSON 存储。虽然是本地工具，但建议至少提供环境变量读取作为替代方案（`process.env.EASYMC_API_KEY`）。
...
- **`DANGEROUS_COMMANDS` 与 `rules.js` 的 `COMMAND_RULES.DANGEROUS`** 重复定义，内容略有不同。`tools.js` 包含 `'whitelist'`（不含参数），`rules.js` 包含 `'gamerule'`。需统一。
- **缺少输入验证**：`execute_mc_command` 不验证命令是否包含特殊字符（如换行符 `\n`）。恶意输入可以注入多条命令。建议在 `sendCommand` 前做 `command.includes('\n')` 检查。
...
- **`sendCommand` 不验证命令内容**：直接将 `command + '\n'` 写入 stdin。如果 command 包含换行符，会变成多条命令。建议做 `command.replace(/[\r\n]/g, ' ')` 或拒绝包含换行的命令。
- **`detectServerReady` 硬编码检测字符串**：`'Done ('` 和 `'For help, type'` 是 Vanilla/Paper 的标准输出。不同核心（Forge、Fabric）的启动完成消息可能不同。建议为每种核心维护一个完成检测模式列表。
- **`resolveJavaPath` 回退到 `'java'`**：如果所有检测都失败，返回字符串 `'java'`，spawn 会在 PATH 中查找。如果 PATH 中没有 java，spawn 会报错。建议在返回前做一次存在性检查。
- **`restart()` 固定等 2 秒**：等待端口释放。在大内存服务器上 stop 后端口释放可能更慢。建议改为轮询端口可用性。
- **`mcVersion` 从日志解析**：正则 `/for Minecraft (\S+)/` 可能匹配到版本号带尾随标点的情况。建议用更精确的正则。
...
- **`logStream` 没有错误处理**：`fs.createWriteStream` 创建的写入流如果磁盘满或权限错误，会触发 `error` 事件但无人处理。建议加 `this.logStream.on('error', ...)`。
- **`clearHistory()` 重新分配整个 buffer**：`this.buffer = new Array(this.maxHistory)` 会触发 GC。可以改为 `this.head = 0; this.count = 0;` 并保持 buffer 不变，但需确保旧数据不被 `_toArray` 读取（当前 `_toArray` 依赖 `count`，所以只需重置 count 即可）。
- **`searchHistory` 线性扫描**：对大缓冲区（5000 条）做线性扫描可接受。如果需要更快的搜索，可以考虑维护一个倒排索引，但对当前规模不值得。
- **`destroy()` 关闭 logStream 但不清空 buffer**：如果 `destroy` 后有人调用 `getHistory`，会返回旧数据。建议在 `destroy` 后设标记并拒绝后续操作。
...
- **`readModMeta` 依赖 `toml` 包**：但 `package.json` 中没有 `toml` 依赖。Forge 的 `mods.toml` 解析会失败并 fallback 到返回 raw 文本。需要添加 `toml` 到 `dependencies` 或改用 `@iarna/toml`。
...
- **`checkPermission` 只检查命令的第一个词**：`command.split(' ')[0]` 作为 cmdBase。但 `whitelist off` 这种两词命令在 blacklist 中是完整匹配的，而 `checkPermission` 只检查 `whitelist`。这导致 `whitelist add` 也会被 blacklist 匹配。需明确黑名单匹配逻辑是前缀匹配还是精确匹配。
- **未匹配命令默认要求 OP 2+**：这是一个安全默认值，好。但可能导致一些安全的命令（如 `seed`）被误拒。`seed` 已在 QUERY 规则中，没问题。但新命令出现时可能被误拒。
- **规则用数组存储**：`rule.commands` 是数组，`includes` 检查是 O(n)。命令数量少，性能可忽略。但建议改为 Set 以表达"集合"语义。
...
- **`downloadJarToManager` 的 `.part` 原子写入**：先下载到 `.part` 文件，完成后 rename。好的设计，避免了部分下载被误认为已完成。但 rename 失败（如目标文件被锁定）会抛异常，此时 `.part` 文件不会被清理。建议在 catch 中删除 `.part`。
...
- **与 `config.js` 的字段不完全一致**：`config.local.json` 有 `java.customPath` 但 `config.js` 没有。这是运行时动态添加的字段被持久化导致的。建议在 `config.js` 默认值中也加上 `customPath: ''`。
...
- **缺少 `toml` 依赖**：`mod-manager.js` 中 `require('toml')` 但 `package.json` 没有列出。需添加或改用已有依赖。
- **`open` 包在 dependencies 中但未使用**：`start.js` 使用 `spawn` 打开浏览器，没有 `require('open')`。可以移除。
...
| 概念 | 定义位置 1 | 定义位置 2 |
|---|---|---|
| 危险命令列表 | `tools.js: DANGEROUS_COMMANDS` | `rules.js: COMMAND_RULES.DANGEROUS` |
...
后端 Socket.IO 事件处理器有些有 try-catch（如 `server:start`），有些没有（如 `plugin:toggle`）。建议统一加 try-catch 并 emit 错误事件。
