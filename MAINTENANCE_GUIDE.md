...
> 生成时间：2026-07-09｜上次维护更新：2026-07-12
 ...
 **`terminal:encoding` Socket 事件**：新增终端编码切换支持。前端 emit `terminal:encoding` 后，调用 `serverManager.setEncoding(encoding)`，成功后重发 `terminal:history`。iconv-lite 解码流在 `server-manager.js` 中重建。
- **`app.get('*')` 通配路由**：将所有非 `/api` 请求交给前端 SPA。需确保新增加的静态资源路径（如字体）在 `express.static` 之前注册。当前顺序正确。

### `server/config.js`（85 行）

**职责**：默认配置定义，用户通过 `config.local.json` 覆盖。

**维护建议**：
- **`agent.ingameMemoryDir`** 支持相对路径和绝对路径。`IngameMemoryStore` 构造函数通过 `path.isAbsolute()` 判断：绝对路径直接使用，相对路径则拼接 `serverDir`。`server-manager.js` 和 `api.js` 中通过 `path.relative` 做了转换，确保一致性。
- **`agent.whisperMaxResponseLength`** 控制 whisper 模式下回复最大长度（默认 128）。与 `maxResponseLength`（默认 30）分开配置，避免记忆内容被截断。
...
**职责**：监听 Minecraft 服务器终端输出，解析玩家聊天，检测 @agent 触发词和 /agentpm 私密消息，分发请求给 AgentCore，按消息类型选择广播或私聊回复。

**维护建议**：
- **`pmRegex` 与 `chatRegex` 并行**：`parseLine` 先尝试匹配 `pmRegex`（`[AgentPM]` 格式），失败再匹配 `chatRegex`（`<player> message` 格式）。如果未来新增更多消息入口（如 Discord 桥接），建议将解析器改为数组遍历模式，统一扩展。
- **回复路由依赖 `isWhisper` 布尔值**：`handleAgentRequest` 根据 `isWhisper` 选择 `tell` 或 `say`。如果未来需要更多回复方式（如 action bar、title），建议改为策略对象而非布尔分支。
...
**职责**：定义 7 个 AI 可调用工具（execute_mc_command、server_manager、file_manager、broadcast_reply、server_properties、backup_world、ingame_memory）及执行逻辑。

**维护建议**：
- **`ingame_memory` 每次调用实例化 `IngameMemoryStore`**：无状态设计，每次调用重新读取 `usercache.json` 和文件系统。对低频调用可接受，但如果记忆操作频率增高（如每个 @agent 请求都读写记忆），建议改为单例 + 缓存。
- **`ingame_memory` 不做写入内容长度限制**：`content` 参数可以是任意长度字符串。如果 AI 生成了异常长的记忆内容，会直接写入文件。建议在写入前加最大长度检查（如 500 字符）。
...
**职责**：定义 Admin 和 Player 的系统提示词，支持通过 `config.prompts` 自定义。包含游戏内记忆规则和私密消息模式提示。
...
- **`{whisperHint}` 动态注入**：根据 `isWhisper` 参数替换为不同的中文提示。如果用户在 `config.prompts.player` 中自定义了 prompt 但没有包含 `{whisperHint}` 占位符，whisper 模式提示会被静默丢弃，玩家不会收到私密模式提醒。建议在文档中说明自定义 prompt 必须包含此占位符。

### `server/agent/ingame-memory-store.js`（358 行）

**职责**：游戏内记忆持久化存储，按玩家 UUID 隔离目录，管理公开/私有记忆文件，支持世界替换时自动归档。

**维护建议**：
- **`_resolveUuid` 每次读文件**：每次 UUID 解析都重新读取 `usercache.json`。在同一个 Agent 工具调用循环中如果多次读写记忆，会重复读取该文件。建议加内存缓存（TTL 或 per-request）。
- **`_readMemoryFile` 的 frontmatter 解析简单**：手写 `key: value` 按行解析。如果 value 中包含冒号（如 `createdAt: 2026-07-11T08:30:00Z`），`indexOf(':')` 取第一个冒号，后续内容作为 value，逻辑正确。但不支持多行值或嵌套结构。当前场景够用。
- **`archiveOldMemory` 使用 `fs.cpSync` + `fs.rmSync`**：先复制再删除，不是原子操作。如果在复制和删除之间崩溃，数据会同时存在于原位和归档中。重启后 `ensureStructure()` 会重建空结构，但旧数据可能残留。建议改为 `fs.renameSync`（原子操作），但跨目录 rename 在某些文件系统上不支持，`cpSync + rmSync` 是安全回退。
- **`isWorldReplaced` 时间对比逻辑**：比较 `level.dat` 的 `mtimeMs` 和 `meta.json` 的 `createdAt`。但如果用户手动修改了 `level.dat`（如用 NBT 编辑器），`mtime` 会更新，可能误判为世界替换。建议在 prompt 或文档中说明此限制。
- **`_writeMemory` 追加格式**：追加时在已有 body 后加 `\n- ${content}`。如果 body 为空，直接写 `- ${content}`。格式正确。但如果 AI 传入的 `content` 本身包含 `- ` 前缀，会出现 `- - xxx` 的双重前缀。建议在写入前 strip 掉前导 `- `。
- **`_cleanupOldArchives` 按 `ctimeMs` 排序**：`ctimeMs` 是文件创建时间。在某些文件系统上 `ctime` 是元数据变更时间而非创建时间。建议改为从目录名（ISO 时间戳）排序，更可靠。

---

## 后端：Manager 层（业务管理器）

### `server/managers/server-manager.js`（331 行）

**职责**：管理 Minecraft 服务器进程的启动、停止、重启、状态跟踪、stdin 通信。启动时自动部署内置插件和检测世界替换。支持终端编码热切换（iconv-lite）。

**维护建议**：
- **构造函数 4 个参数**：`config, javaManager, terminalManager, pluginAutoDeployer`。最后一个为可选参数（默认 null），向后兼容。但如果未来继续添加可选依赖，参数列表会变得冗长。建议改为 options 对象模式：`constructor(config, managers)`。
- **`archiveMemoryIfWorldReplaced` 每次启动都执行**：即使世界没有被替换，也会实例化 `IngameMemoryStore` 并调用 `ensureStructure()` 和 `isWorldReplaced()`。开销很小（几个文件 stat），但日志中不会留下"未检测到替换"的信息，调试时可加 verbose 日志。
...
- **`_setupTerminalStreams()` 提取方法**：iconv-lite 解码流 + readline 接口封装为独立方法，供 `start()` 和 `setEncoding()` 复用。每次切换编码时关闭旧 reader、重建新流。
- **`setEncoding(encoding)` 热切换**：返回 boolean 表示是否实际变更。`index.js` 中 Socket handler 据此决定是否重发 terminal history。
- **`archiveMemoryIfWorldReplaced` 处理 skipped**：`archiveOldMemory()` 返回 `{ skipped: true }` 时不再输出 undefined 路径，改为静默日志。

### `server/managers/plugin-auto-deployer.js`（59 行）

**职责**：服务器启动或 JAR 替换时，检测核心是否支持插件，自动从 `.easymc/plugins/` 复制内置插件到 `mc-server/plugins/`。

**维护建议**：
- **无条件覆盖已有插件**（设计意图）：`deployIfSupported()` 使用 `copyFileSync` 直接覆盖目标文件。这是期望行为——`.easymc/plugins/` 被视为内置插件的唯一权威源，确保每次启动后 `mc-server/plugins/` 中的内置插件与源一致。如果用户手动修改了同名插件，会被还原。如果需要用户自定义版本，应改用不同文件名。
- **`builtinPluginsDir` 可配置**：默认从 `<cwd>/.easymc/plugins/` 读取，可通过 `config.easymc.builtinPluginsDir` 覆盖。
- **无部署结果通知**：返回 `{ deployed, copied }` 对象但没有 emit 事件。Web UI 无法感知插件部署状态。如果需要 UI 展示，应集成到 Socket.IO 事件系统。
...
**职责**：Express REST API 路由，提供配置管理、服务器状态、Java 检测、工作区文件列表、Agent 工具/提示词管理、下载搜索/安装、游戏内记忆归档等接口。
...
- **`POST /server/archive-memory` 手动归档**：调用 `IngameMemoryStore.archiveOldMemory()`。`archiveOldMemory()` 会在归档前检查是否有实际玩家数据，空目录时跳过归档并返回 `{ skipped: true }`。
...
### `client/src/App.jsx`（595 行）

**职责**：应用主组件（shell），管理 Socket.IO 状态中枢、sidebar 导航、页面路由。所有页面组件已提取到独立文件，通过 props-down 模式接收数据。

**维护建议**：
- **已从 2838 行拆分至 595 行**：6 个页面域（Chat/Console/Download/Resource/Setup/Agent）均已独立。App.jsx 保留的是 App 层级固有职责——state 中枢、Socket 事件总注册、sidebar JSX、页面路由。
- **状态管理**：App 组件持有所有核心状态（serverStatus、terminalLines、chatSessions 等）并通过 props 分发给子组件。自定义 Hook（`useDownloadQueue`、`useAgentStream`）已提取到 `hooks/` 目录。
- **Socket 事件监听**：master useEffect 在单处注册 15+ 事件监听器。虽然事件量大，但集中管理避免了遗漏清理。
- **`terminalEncoding` 同步**：useEffect 监听 `terminalEncoding` 状态变化，emit `terminal:encoding` 到后端触发 iconv-lite 解码流重建。
- **缺少 ErrorBoundary**：任何子组件抛异常仍会导致白屏。建议在 `main.jsx` 加全局 ErrorBoundary。
- **缺少代码分割**：所有组件在同一 bundle 中。建议使用 `React.lazy` + `Suspense` 做懒加载。
...
### `client/src/hooks/`（3 个 Hook）

**文件**：`useSocket.js`、`useDownloadQueue.js`、`useAgentStream.js`

**职责**：Socket.IO 连接管理、下载队列管理、Agent Stream 消息管理。

**维护建议**：
- **`useSocket.js`**：Socket 单例在模块级创建，`on` 返回清理函数。`emit` 不支持 Promise/ack，如需服务端确认建议加 `emitWithAck` 方法。
- **`useDownloadQueue.js`**：从 App.jsx 提取的下载队列 Hook，管理下载进度和状态。
- **`useAgentStream.js`**：从 App.jsx 提取的 Agent Stream Hook，管理 stream 消息数组和连接状态。

### `client/src/utils/`（3 个工具文件）

**文件**：`download.js`、`format.js`、`fuzzySearch.js`

**职责**：下载资源装饰器、格式化辅助函数、模糊搜索算法。从 App.jsx 中提取的纯函数。

### `client/src/constants/app.js`

**职责**：全局常量定义（`DOWNLOAD_TYPES`、`DOWNLOAD_SOURCE_OPTIONS`、`DOWNLOAD_RESOURCES`、`WELCOME_MESSAGES` 等）。从 App.jsx 中提取。

### `client/src/components/` 目录结构

已按功能域拆分为 7 个子目录：

| 目录 | 包含组件 | 职责 |
|---|---|---|
| `agent/` | AgentStreamPage | Agent 实时活动流 |
| `chat/` | ChatPage | AI 聊天界面与会话管理 |
| `console/` | ConsolePage | 服务器终端、控制、状态面板 |
| `download/` | DownloadCenterPage, DownloadDetailsPanel, DownloadFilterButton, DownloadResultItem, DetailStat, ResourceIcon, SourceBadge | 下载中心（搜索、筛选、文件详情、安装） |
| `resource/` | ResourcePage, ResourceCard | Mods/Plugins 资源管理列表 |
| `setup/` | BasicSetupPage | 服务器基础设置（核心、Java、游戏模式、AI 配置） |
| `ui/` | PageShell | 通用页面壳组件 |
...
游戏内记忆系统涉及多个文件的协作，完整数据流如下：

```
游戏内消息:
  <playerA> @agent 记住我喜欢橡木           → chat-monitor.js (chatRegex, isWhisper=false)
  playerA: /agentpm 我的秘密基地在(100,20,300) → [AgentPM] 日志 → chat-monitor.js (pmRegex, isWhisper=true)

chat-monitor.js:
  handleAgentRequest(playerName, request, isWhisper)
    → agent-core.js: handlePlayerRequest(playerName, request, permission, isWhisper)
      → getContext(onConfirm, playerName, isWhisper) → context.callerPlayerName
      → getPlayerPrompt(..., isWhisper) → 注入 {whisperHint}
      → tools.js: executeTool('ingame_memory', args, context)
        → ingame-memory-store.js: readPublic/readPrivate/writePublic/writePrivate
          → usercache.json (name↔UUID)
          → mc-server/ingame_memory/<uuid>/public.md | private.md

回复路由:
  isWhisper=true  → serverManager.sendCommand('tell playerName ...')
  isWhisper=false → serverManager.sendCommand('say [Agent]: ...')

世界替换:
  server-manager.js: start() → archiveMemoryIfWorldReplaced()
    → ingame-memory-store.js: isWorldReplaced() → archiveOldMemory()

手动归档:
  POST /server/archive-memory → api.js → ingame-memory-store.js: archiveOldMemory()

插件部署:
  server-manager.js: start() → pluginAutoDeployer.deployIfSupported()
    → core-detector.js: detect() → supportsPlugins?
    → .easymc/plugins/*.jar → mc-server/plugins/*.jar
```
...
后端 `plugin-manager.js` 和 `mod-manager.js` 代码结构完全相同。应提取后端 `JarManager` 基类。前端已通过统一的 `ResourcePage` 组件合并了 Mods/Plugins 列表视图，消除了前端重复。

### 3. 前端组件拆分（已完成）

`App.jsx` 已从 2838 行拆分至 595 行，6 个页面域已提取到独立目录。当前结构：
- `components/agent/` → AgentStreamPage
- `components/chat/` → ChatPage
- `components/console/` → ConsolePage
- `components/download/` → DownloadCenterPage + 6 个子组件
- `components/resource/` → ResourcePage + ResourceCard
- `components/setup/` → BasicSetupPage
- `components/ui/` → PageShell
- `hooks/` → useSocket, useDownloadQueue, useAgentStream
- `utils/` → download, format, fuzzySearch
- `constants/` → app.js

App.jsx 保留 App 层级的固有职责（state 中枢、Socket 总注册、sidebar、路由）。
...
- `IngameMemoryStore` 的读写和隐私鉴权测试（安全关键）
- `ChatMonitor` 的 `chatRegex` / `pmRegex` 解析测试（正确性关键）
