...
const DOWNLOAD_TYPES = ['All', 'Server Core', 'Mods', 'Plugins'];
      ...
      <div className="flex h-full gap-4 p-4">
        <aside className="w-72 shrink-0 rounded-[28px] bg-md-surface shadow-[0_4px_16px_rgba(62,0,30,0.10)] dark:shadow-[0_4px_18px_rgba(0,0,0,0.30)] flex flex-col p-4 gap-4 overflow-hidden">
          <div className="rounded-full bg-md-surfaceVariant/60 p-1 flex relative">
            <div
              className={`absolute top-1 bottom-1 w-[calc(50%-4px)] rounded-full bg-md-primaryContainer transition-all duration-300 ${
          ...
          {mode === 'agent' ? (
            ...
            <nav className="flex-1 flex flex-col gap-1 overflow-y-auto">
              ...
              <SidebarButton activeClass={activeNav('server-mods')} onClick={() => setPage('server-mods')} icon={Puzzle} count={mods.length}>
                Mods
              </SidebarButton>
              <SidebarButton activeClass={activeNav('server-plugins')} onClick={() => setPage('server-plugins')} icon={Package} count={plugins.length || '-'}>
                Plugins
              </SidebarButton>
          ...
          <div className="rounded-[28px] bg-md-surfaceContainer/85 px-5 py-4 text-sm text-md-outline shadow-sm">
            ...
            <div className="mt-3 flex items-center gap-3 text-md-onPrimaryContainer">
...
function ThemeToggleButton({ theme, onToggle }) {
  ...
  return (
    <button
      onClick={onToggle}
      className="flex h-16 w-16 items-center justify-center rounded-full bg-md-primaryContainer text-md-onPrimaryContainer shadow-sm transition hover:scale-[1.03] hover:shadow-md focus:outline-none focus-visible:outline-none active:scale-95"
    ...
    <section className="material-page h-full flex flex-col overflow-hidden">
      {isEmpty ? (
        <div className="flex-1 flex flex-col items-center justify-center p-6">
          <h1 className="text-4xl md:text-5xl font-medium text-md-primary text-center mb-8">Hello! What can I do for you?</h1>
          <ChatInput value={chatInput} onChange={setChatInput} onSubmit={sendMessage} autoFocus />
        </div>
      ) : (
        <>
          <div className="flex items-center justify-between pb-4">
            <div>
              <h1 className="text-3xl font-medium text-md-primary">{activeSession?.title || 'Chat'}</h1>
              <p className="text-xs text-md-outline mt-1">{messages.length} messages · {formatDate(activeSession?.updatedAt)}</p>
            </div>
            <div className="flex items-center gap-2">
              {(isTyping || pendingConfirm) && (
                <button
                  onClick={interruptAgent}
                  disabled={agentStopping}
                  className="rounded-full bg-md-errorContainer px-4 py-2 text-xs font-bold text-md-error transition hover:bg-md-errorContainer/80 disabled:opacity-60 flex items-center gap-2"
                  title="Stop current Agent run"
                >
                  <XCircle size={15} /> {agentStopping ? 'Stopping...' : 'Stop Agent'}
                </button>
              )}
            </div>
          </div>
          ...
          <div className="pb-2">
            <ChatInput value={chatInput} onChange={setChatInput} onSubmit={sendMessage} />
          </div>
        </>
      )}
    </section>
...
function DownloadCenterPage() {
  ...
  const selectedQueued = selectedResource ? queue.some(item => item.projectId === selectedResource.sourceId || item.id === selectedResource.id) : false;
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[280px_minmax(0,1fr)_320px] gap-4 min-h-0 flex-1">
          ...
          <div className="flex-1 overflow-y-auto space-y-3 pr-1">
            {filteredResources.length === 0 ? (
              <div className="h-full rounded-[24px] border border-dashed border-md-surfaceVariant bg-md-surfaceContainer flex flex-col items-center justify-center text-center p-8">
                ...
                <p className="text-sm text-md-outline mt-1">Try All types, All sources, or a shorter search term.</p>
              </div>
            ) : (
              filteredResources.map(resource => (
                <DownloadResultItem
                  ...
                  queued={queue.some(item => item.projectId === resource.sourceId || item.id === resource.id)}
        ...
        <aside className="min-h-0 overflow-y-auto space-y-3">
          ...
          <DownloadQueuePanel
            ...
            onClear={async () => {
              try {
                const response = await fetch('/api/download/queue/clear', { method: 'POST' });
                const data = await response.json();
                setQueue(data.queue || []);
              } catch {
                setQueue([]);
...
function DownloadResultItem({ resource, active, queued, onSelect, onQueue }) {
  ...
  return (
    ...
    >
      <div className="flex items-start gap-4">
        <button
          onClick={onSelect}
          className="h-14 w-14 rounded-2xl bg-md-primaryContainer text-md-onPrimaryContainer flex items-center justify-center shrink-0"
        ...
        >
          {queued ? <CheckCircle size={14} /> : <ChevronRight size={14} />}
          {queued ? 'Queued' : 'Details'}
        </button>
      </div>
    </article>
  );
...
function DownloadDetailsPanel({
  ...
  return (
    <section className="rounded-[24px] bg-md-surfaceContainer border border-md-surfaceVariant p-5 shadow-sm">
        ...
        {files.length === 0 ? (
          ...
          <div className="space-y-2 max-h-44 overflow-y-auto pr-1">
            {files.slice(0, 8).map(file => (
              <label
                key={file.fileId}
                className={`flex items-start gap-3 rounded-[18px] border px-3 py-2 cursor-pointer ${
      ...
      <button
        onClick={onQueue}
        disabled={queued || installing || filesLoading}
        className="mt-4 w-full rounded-full bg-md-primary px-4 py-3 text-sm font-bold text-md-onPrimary flex items-center justify-center gap-2 disabled:bg-md-surfaceVariant disabled:text-md-outline"
      >
        {installing ? <Loader2 size={16} className="animate-spin" /> : queued ? <CheckCircle size={16} /> : <Download size={16} />}
        {installing ? 'Installing...' : queued ? 'Added to queue' : resource.type === 'Server Core' ? 'Core install requires confirmation' : 'Install selected file'}
      </button>
...
function DownloadQueuePanel({ queue, onClear }) {
  return (
    <section className="rounded-[24px] bg-md-surfaceContainer border border-md-surfaceVariant p-5 shadow-sm">
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-xs text-md-outline">Queue</p>
          <h2 className="text-lg font-bold">{queue.length} item{queue.length === 1 ? '' : 's'}</h2>
        </div>
        <button
          onClick={onClear}
          disabled={queue.length === 0}
          className="rounded-full px-3 py-1.5 text-xs font-bold bg-md-surfaceVariant/65 text-md-outline disabled:opacity-50"
        >
          Clear
        </button>
      </div>
      ...
      <div className="mt-4 space-y-3">
        {queue.length === 0 ? (
          <div className="rounded-[18px] bg-md-bg border border-dashed border-md-surfaceVariant px-4 py-5 text-sm text-md-outline text-center">
            Installed resources and failed attempts will appear here.
          </div>
        ) : (
          queue.map(item => (
            <div key={item.id} className="rounded-[18px] bg-md-bg border border-md-surfaceVariant px-4 py-3">
              <div className="flex items-center justify-between gap-3">
                <p className="text-sm font-bold truncate">{item.name}</p>
                <span className="text-xs font-bold text-md-success">{item.progress}%</span>
              </div>
              <div className="mt-2 h-2 rounded-full bg-md-surfaceVariant overflow-hidden">
                <div className="h-full rounded-full bg-md-success" style={{ width: `${item.progress}%` }} />
              </div>
              <p className="text-[11px] text-md-outline mt-2">{item.target || 'downloads/'} - {item.status}</p>
            </div>
          ))
        )}
      </div>
