...
class ChatSessionManager {
  ...
  renameSession(id, title) {
    ...
    const nextTitle = this.titleFromMessage(title);
    session.title = nextTitle || 'New chat';
  ...
  addUsage(id, usage = {}) {
    const session = this.getSession(id);
    if (!session || !usage) return session;
    ...
    session.usage = {
      prompt_tokens: current.prompt_tokens + Number(usage.prompt_tokens || 0),
      completion_tokens: current.completion_tokens + Number(usage.completion_tokens || 0),
